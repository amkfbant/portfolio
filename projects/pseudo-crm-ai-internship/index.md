...
このプロジェクトの差別化点は「動かして見せられるリアル風データ」を**生成スクリプトで再現可能**に作っていること。手書き JSON ではなく、**業種・規模・地域・課題・予算・タイムライン・決裁構造・CRM 活動履歴が論理的に整合した 200 社分の商談データセット**を build している。

- **業種バリエーション 200 種**: 採用 SaaS / 精密部品製造 / 歯科グループ / D2C 化粧品 / 物流営業 / 不動産管理 / 教育 BtoB ... と実在業界を満遍なくカバー。各社で `industry` / `subIndustry` / `salesModel` を業種に合わせた言葉で記述。
- **会社内で論理整合**: `contacts[]` の persona（decision_maker / champion）と `decisionProcess.decisionMakers` / `approvalFlow` が一致。`pain_points` → `goals` → `requirements` が一直線で読める。
- **時系列の一貫性**: opportunity stage history / CRM activities / メール open・click イベント / pipeline の節目時刻が、すべて時系列で破綻なく繋がる。
- **数値の現実性**: 月間リード → 商談 → 受注の数値が「商談化率 → 受注率」で逆算しても破綻せず、予算帯（中堅 SaaS = 月 80〜120 万、中小製造 = 月 30〜50 万）と従業員数・売上レンジが整合。
- **6 デモを貫通する同一企業 ID**: 1 社を Demo 01 でリスト化 → Demo 03 で商談メール → Demo 04 で提案書まで一気通貫で見せられる。
- **音声まで生成**: Demo 06 の通話採点用に、scripts でテキスト → 音声合成（TTS） → 文字起こし（Whisper 系）まで生成パイプラインを組み、見た目だけのモックではなく音声 → transcript → 採点の流れを丸ごと再現できる。
- **データセット世代管理**: 各 JSON に `datasetName` / `version` / `recordCount` / `generatedAt` を持たせ、生成スクリプト再実行で安全に世代更新可能。

**ポートフォリオに掲載のデモ動画は、上記 200 件のシードデータをそのまま擬似 CRM に投入し、Demo 04 をライブ動作させたものです。動画前後で AI に渡している顧客データ・商談ヒアリングは、すべてこの擬似 CRM 経由で参照されています。**
